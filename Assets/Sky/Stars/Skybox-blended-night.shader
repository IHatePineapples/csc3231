...
My fork of the builtin  shader, main differences are :
 - Always render at max resolution (i.e. HQ sun)
 - Night mode: At night, render a cubebox shader
 - Add rotation
 - Add dimming of cubmap depending on time of day

*/

Shader "Skybox/Blended-Night" {
Properties {
    ...
    _Roatation ("Rotation", Range(0,360)) = 0
}
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                ...
                half alt = normalize(_WorldSpaceLightPos0.xyz).y;
                half3 c = DecodeHDR(tex, _Tex_HDR) * (alt < 0 ? -alt : 0);
                col = c * _SkyTint.rgb * unity_ColorSpaceDouble.rgb;
                col *= _Exposure;
